...
function runtimeField<T>() {
  return z.custom<T>().transform(() => undefined);
  //  ctx.addIssue({code: 'custom', message: 'This field is runtime-only and should not appear in a user-defined config'});
  //});
}
...
export const ResourceZod = z.object({
  ...
  element: runtimeField<HTMLImageElement>(),
});
...
export function encodeUrlWithConfig(configFull: string): string {
  ...
  // TODO: убирать лишнюю говнину, предупреждать о лишних полях
  // stddev NaN (1 hit?)
  // precision 2 не везде. А хотя вообще везде до целых округлять?
  // знак %
  // отображение таблицами
  // добавить возможность скачивать с номером/именем испытуемого в названии файла
  return fullUrl;
}
